    ...
    <div v-if="!showCamera">
      ...
      <div v-else class="pl-6 pr-6 pt-7 pb-4">
        <div class="bg-[#B9C8EA] pt-3 pb-3 pl-4 pr-3 rounded-t-md flex gap-3">
          ...
          <p class="text-[#4A6BB6] font-[600] font-[Inter]">
            {{ projectName }}
          </p>
        </div>
        <div
...
async function insertCheckIns(latitude, longitude) {
  let time =
    actionName.value === 'Check-Out'
      ? timesheetEntry.value.time_logs[0].from_time
      : timesheetEntry.value.time_logs[0].to_time
...
async function handleImageCapture(file) {
  ...
  if (cameraMode.value === 'Check-In') {
    ...
    await uploadAllAttachments('Timesheet', timesheetDetails.value.name, [
      imageFile.value,
    ])
    if (cameraMode.value === 'Upload') {
      additionalImage()
...
function additionalImage() {
  let note =
    '<br/><p>Additional Photo added at ' +
    dayjs().format('hh:mm:ss A') +
    (caption.value ? ' with notes : ' + caption.value : ' with on notes') +
    '</p>'
